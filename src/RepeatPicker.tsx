import { REPEAT_LABELS, type RepeatInterval, type RepeatUnit, type RepeatMonthMode } from './contexts/TodoContext';
...
export const EMPTY_REPEAT: RepeatState = { repeat: '', repeatInterval: 1, repeatUnit: 'day', repeatMonthMode: 'same_date' };
...
// den betingede visningen av intervall+enhet (og, for måned, dato-modus) for
// "Egendefinert". repeat/repeatInterval/repeatUnit/repeatMonthMode bundles i
// én verdi/onChange (som TimePicker), slik at hele grenen for "custom" kun
// finnes ett sted i stedet for duplisert i to skjemaer.
export default function RepeatPicker({ value, onChange, disabled }: {
  value: RepeatState;
  onChange: (next: RepeatState) => void;
  disabled?: boolean;
}) {
  const { repeat, repeatInterval, repeatUnit, repeatMonthMode } = value;
    ...
    <div>
      ...
      {repeat === 'custom' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginTop: 6 }}>
          ...
          {repeatUnit === 'month' && (
            <select className="input" value={repeatMonthMode} disabled={disabled}
              onChange={e => onChange({ ...value, repeatMonthMode: e.target.value as RepeatMonthMode })}
              style={{ width: '100%' }}>
              <option value="same_date">Samme dato</option>
              <option value="last_day">Siste dag i måneden</option>
            </select>
          )}
        </div>
      )}
